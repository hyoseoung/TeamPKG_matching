...
@WebServlet("/matchBoard/edit.do")
public class EditController extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		String idx = req.getParameter("idx");
		MBoardDAO dao = new MBoardDAO();
		MBoardDTO dto = dao.getView(idx); //회원정보 불러오기
		req.setAttribute("dto", dto);
		req.getRequestDispatcher("/MatchBoard/Edit.jsp").forward(req, resp);
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(result==1) {
			...
			resp.sendRedirect("../matchBoard/view.do?idx="+idx);
		}else {
			AlertFunction.alertLocation(resp, "수정이 되지 않았습니다.", "../matchBoard/view.do?idx="+idx);
		}
